...
async function main() {
  ...
  for (const row of sites.rows as unknown as Array<{ id: string; slug: string; name: string; domain: string; created_at: string }>) {
  ...
  for (const row of users.rows as unknown as Array<{ email: string; name: string; role: string; invited_at: string; last_login_at: string | null; site_slug: string }>) {
  ...
  for (const row of creds.rows as unknown as Array<{ email: string; password_set_at: string; password_changed_at: string | null }>) {
